...
function M.setup(opts)
  ...
  if not M.has() then
    require("lazy.core.util").error(
      "**LuaVim** needs **lazy.nvim** version "
        .. M.lazy_version
        .. " to work properly.\n"
  ...
  if vim.fn.argc(-1) == 0 then
    ...
    vim.api.nvim_create_autocmd("User", {
      group = vim.api.nvim_create_augroup("LuaVim", { clear = true }),
      pattern = "VeryLazy",
      callback = function()
        M.load("autocmds")
        M.load("keymaps")
...
function M.load(name)
  ...
  -- always load LuaVim, then user file
  if M.defaults[name] or name == "options" then
    _load("config." .. name)
  end
  _load("config." .. name)
  if vim.bo.filetype == "lazy" then
    -- HACK: LuaVim may have overwritten options of the Lazy ui, so reset this here
